...
<template>
  <div p4 grid="~ cols-[max-content_1fr]" gap-8 items-center ma max-w-300>
    <NuxtImg width="400" height="600" format="webp" :src="`/tmdb${props.item.poster_path}`"
      :alt="props.item.title || props.item.name" block border="4 gray4/10" w-79 lt-md:hidden transition duration-400
      object-cover aspect="10/16" :style="{ 'view-transition-name': `item-${props.item.id}` }" />
        ...
        <ul grid="~ cols-[max-content_1fr] lg:cols-[max-content_1fr_max-content_1fr] gap3" items-center>
          ...
          <template v-if="directors?.length">
            ...
            <div flex="~ row wrap gap1">
              <NuxtLink v-for="person of directors" :key="person.id" :to="`/person/${person.id}`"
                bg="gray/10 hover:gray/20" p="x2 y1" rounded text-xs>
                {{ person.name }}
              </NuxtLink>
            </div>
          </template>
          ...
          <template v-if="props.item?.genres?.length">
            ...
            <div flex="~ row wrap gap1">
              <NuxtLink v-for="genre of props.item.genres" :key="genre.id" :to="`/genre/${genre.id}/${type}`"
                bg="gray/10 hover:gray/20" p="x2 y1" rounded text-xs>
                {{ genre.name }}
              </NuxtLink>
            </div>
