...
<template>
  <div :key="item.id" relative class="aspect-ratio-3/2 lg:aspect-ratio-25/9" bg-black>
    <div absolute top-0 right-0 lt-lg="left-0" lg="bottom-0 left-1/3">
      <NuxtImg width="1220" height="659" format="webp" :src="`/tmdb${props.item.backdrop_path}`"
        :alt="props.item.title || props.item.name" h-full w-full object-cover />
    </div>
      ...
      <Transition appear name="hero">
        <div v-show="mounted">
          ...
          <div v-if="trailer" class="py5 display-none lg:block">
            <button type="button" flex="~ gap2" items-center p="x6 y3" bg="gray/15 hover:gray/20" transition
              :title="$t('Watch Trailer')" @click="playTrailer()">
              <div i-ph-play />
              {{ $t('Watch Trailer') }}
            </button>
          </div>
    ...
    <div v-if="trailer" lg:hidden absolute left-0 top-0 right-0 h="2/3" items-center justify-center>
      <button type="button" items-center p10 text-5xl op20 hover:op80 transition :title="$t('Watch Trailer')"
        @click="playTrailer()">
