...
<template>
    <div>
        <div flex bg-gray:10 items-center px6 py4 gap3 sticky>
            ...
            <input v-model="input" v-focus type="text" text-2xl bg-transparent outline-none w-full
                :placeholder="$t('Type to search...')" @keyup.enter="search">
        </div>

        <div v-if="error" p8 flex flex-col gap3 items-start>
            <h1>{{ $t('Error occurred on fetching') }}</h1>
            <pre py2>{{ error }}</pre>
        ...
        <MediaAutoLoadGrid v-else-if="currentSearch" :key="currentSearch" :fetch="fetch" :items="items" :count="count"
            :blocking="false" type="movie">
